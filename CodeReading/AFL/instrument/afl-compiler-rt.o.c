 ...
 * @note (zys)
 * 
 * pls read all comment marked by @note
 * 
 * Prerequisits:
 *   - weak symbol in linking stage
 *   - thread local storage
 * 
 * Variables:
 *   - __afl_area_*: variable about shared memory used for coverage bitmap
 *                   function `__afl_trace` changes __afl_area_ptr 
 *                   shm_id is passed by env: SHM_ENV_VAR
 *                   shm_len is passed by env: AFL_MAP_SIZE
 *   - ?FORKSRV_FD: file decriptor of pipe taking to afl-fuzz.
 */
...
/***
 * @note: (zys) attribute `weak` 
 *    weak symbol can be overwritten by strong (default) symbol during linkage 
 */
...
#else
/***
 * @note: (zys) C11 thread_local, thread local storage , `__thread`
...
/*** 
 * @note: (zys) trampoline function
 *       x is the current block locations
 */
...
static void send_forkserver_error(int error) {
  ...
  /*** 
   * @note: (zys) FORKSRV_FD
   *    simplex channel talking to fork server,
   *    defined in config.h 
   * 
...
static void __afl_map_shm_fuzz() {
  ...
  /*** @note: (zys) SHM FUZZ ID
   *     if shared memory is used, then its the shared memory id,
   *        see function`shmget`
   *      defined in config.h
  ...
  if (id_str) {
    ...
    __afl_fuzz_len = (u32 *)map;  /*** @note: (zys) fat pointer,  first 4 byte is the len of shm */
...
/**
 * @note (zys) set up shared memory for the coverage bitmap 
 * 
 */
...
/**
 * @note
 * when this function is called the parent process
 *   keeps sprawning child processes that actually does the
 *   run.
 * the server, i.e., parent process will not exit this function
 * the child returns from this function and runs target codes.
 * 
 */
