...
/**
 * @note: just for syntax highlight in vscode
 * 
 */
...
class AFLCoverage : public ModulePass {
 ...
 protected:
  uint32_t    ngram_size = 0;  // AFL_LLVM_NGRAM_SIZE, AFL_NGRAM_SIZE
  uint32_t    ctx_k = 0;       // AFL_LLVM_CTX_K, AFL_CTX_K
  uint32_t    map_size = MAP_SIZE;
  uint32_t    function_minimum_size = 1;
  const char *ctx_str /* AFL_LLVM_CTX */ = NULL, *caller_str /* AFL_LLVM_CALLER */ = NULL, *skip_nozero = NULL;
...
/***
 * @note: (zys) the pass function, where instrument happens
 */ 
bool AFLCoverage::runOnModule(Module &M) {


  /*** 
   *  @note: (zys) LLVMContext 
   *    It (opaquely) owns and manages the core "global" data 
   *    of LLVM's core infrastructure,
  ...
  */
  ...
  char *       inst_ratio_str = getenv("AFL_INST_RATIO");  /*** @note: (zys) prob of instrument a branch */
  ...
  /*** @note: (zys) AFL N-Gram Branch Coverage
   * 
   *    @see https://github.com/AFLplusplus/AFLplusplus/blob/stable/instrumentation/README.ngram.md
   *    
   */ 
   ...
   * 
   * 
   *  - AFLMapPtr: __afl_area_ptr, points to coverage bitmap
   *  - AFLContext: __afl_prev_ctx, type: i8*
   *  - AFLPrevLoc: __afl_prev_loc, n-gram coverage, type: <IntLocTyp, (n_gram - 1)>
   *  - AFLPrevCaller: __afl_prev_caller, context coverage, type: <IntLocTyp, ctx_k>
   * 
   */ 
  ...
  for (auto &F : M) {
    ...
    for (auto &BB : F) {
       ...
       *   returns the first instruction iter that can insert
       */
      ...
      if (instrument_ctx && &BB == &F.getEntryBlock()) {
        ...
        if (ctx_k) {
          ...
          PrevCaller->setMetadata(M.getMDKindID("nosanitize"),
                                  MDNode::get(C, None));
          // xor all elems of __afl_prev_caller 
          //   and cast result to i32.
          PrevCtx =
        ...
        if (has_calls) {
          ...
          if (ctx_k) {
            ...
            /*** @note: (zys)
             * create a `shufflevector` instruction, which
             *    constructs a permutation of elements from two input vectors
             *   
             * @args: input_vec1, input_vec2, shuffle_mask
             * @return: for each pos shuffe_mast select elem to the result
             *         from either 2 input vecs
             * 
             * 
             * here it just shift the prev_caller
             */ 
